...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if self.pruned_heads:
            ...
            # print(f"not pruning heads")
            position_bias_masked = position_bias

        # print(f"pre contig, stride {position_bias_masked.stride()=}")

        # position_bias_masked = position_bias_masked.contiguous()
        # print(f"stride pos bias masked {position_bias_masked.stride()=}")
        # print(f"qk strides {query_states.stride()=}, {key_states.stride()=}")

        # assert position_bias_masked.is_contiguous(), f"position_bias_masked is not contiguous...{position_bias_masked.shape=}"
        # print(f"{position_bias_masked.shape=}, \n{query_states.shape=},{key_states.shape=}\n")
        position_bias_aligned = None
        ...
        if position_bias_masked.stride(-1) != 1:
            position_bias_masked = torch.empty_like(
                mask, memory_format=torch.contiguous_format
            ).copy_(mask)
        # else:
        #    position_bias_aligned = position_bias_masked
        ...
        if use_mem_eff:
            ...
            # mem eff - query_states.shape=torch.Size([1, 16, 10, 64])
            # attn_output = Fa.scaled_dot_product_attention(query_states, key_states, value_states, attn_mask = position_bias_masked, scale=1)
            # mem eff output = attn_output.shape=torch.Size([1, 16, 10, 64])
            # stop = time.perf_counter()
            # total_time = round(stop - start, 6)
            # print(f"total time for mem eff attn {total_time}")
            attn_output = unshape(attn_output)

        else:
            # --- manual attention
            # compute scores
            # print(f"NOT Using MEM EFF")
            scores = torch.matmul(
            ...
            # stop = time.perf_counter()
            # total_time = round(stop - start, 6)
            # print(f"total time for manual attn {total_time}")
            attn_output = unshape(
